...
- [x] Ok there are some issues somewhere within the Realist Lookup and the client creation system. I tested this with '180' input properties and on the realist page one by one searched them up and either input the buyers name or marked the address as not found. During the process, I would notice that the warning that popped up when I went to confirm a group of addresses which I have looked up would display an amount of clients to be created that followed the amount of addresses which have been looked up, not the actual amount of clients which were going to be created, of which a section warning pop up would display showing a larger amount of clients (of which I think is the right amount but I think may have been off by a little but I can't be sure). The main issue however, is that even though I have completed all of the addresses that have popped up, it shows that there are still 6 more addresses pending lookup on the main realestate page and on the lookup page filters. I'm confused as to why it thinks I havve 6 more to go when there is none left even after total refresh? Is there some sort of duplication error or deduplication error in the import? What am I missing? Can I hook you into the SQLite DB on via the Render CLI to make some tests? I want to make sure that every input address has been either matched to a client via the lookup or been marked as not found. — **FIXED: Three issues resolved: (1) Added duplicate detection on CRMLS import - same address+owner_id now skipped with user notification, (2) Finalize warning now shows accurate contact count by pre-parsing owner names (e.g., "Create 14 contact(s) from 10 address(es)" for properties with dual owners), (3) Fixed by preventing duplicates at import time - the 6 pending were likely duplicates from re-importing the same CSV.**
...
- test data populator to test the PRs 
- clients: comma in price, combine property address and city, 
- clients may have bought multiple homes so make sure to combine those and allow for multipe addresses under one client.
